...
export class TextureAtlas {
  ...
  private registerAll() {
    ...
    // More food
    this.reg('cookie', (c, s) => { noisy(c, s, '#c98a3a', 1120, 0.1, 0.4); px(c, s, 3, 6, 10, 4, '#c98a3a'); });
    this.reg('melon_slice', (c, s) => { noisy(c, s, '#e84a4a', 1121, 0.08, 0.3); px(c, s, 2, 3, 12, 2, '#5a9a3a'); });
    this.reg('carrot', (c, s) => { px(c, s, 7, 8, 2, 7, '#e88a3a'); px(c, s, 6, 5, 4, 3, '#5a9a3a'); });
    this.reg('potato', (c, s) => { noisy(c, s, '#c4a46a', 1122, 0.1, 0.4); });
    this.reg('baked_potato', (c, s) => { noisy(c, s, '#8a6a3a', 1123, 0.1, 0.4); });
    // Materials
    this.reg('bone', (c, s) => { noisy(c, s, '#e8e8d8', 1124, 0.06, 0.3); });
    this.reg('gunpowder', (c, s) => { noisy(c, s, '#3a3a3a', 1125, 0.15, 0.5); });
    this.reg('sugar', (c, s) => { noisy(c, s, '#f4f4f4', 1126, 0.06, 0.3); });
    this.reg('paper', (c, s) => { noisy(c, s, '#f0e8d0', 1127, 0.04, 0.2); });
    this.reg('book', (c, s) => { noisy(c, s, '#b3312c', 1128, 0.08, 0.3); });
    this.reg('iron_nugget', (c, s) => { noisy(c, s, '#d8d8d8', 1129, 0.08, 0.3); });
    this.reg('gold_nugget', (c, s) => { noisy(c, s, '#f4d644', 1130, 0.08, 0.3); });
    this.reg('spawn_grazer', (c, s) => { noisy(c, s, '#9c6b3c', 1131, 0.1, 0.4); });
    this.reg('spawn_stalker', (c, s) => { noisy(c, s, '#3a5a3a', 1132, 0.1, 0.4); });
  }
}
